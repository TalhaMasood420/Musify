...
public class ListenLater extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerView = findViewById(R.id.recyclerViewListenLater);
    ...
    public void getData(){
        db.collection("ListenLater").document(FirebaseAuth.getInstance().getCurrentUser().getUid()).collection("ListenLater").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    musicAdapter = new MusicAdapter(ListenLater.this,musicList);
                    recyclerView.setAdapter(musicAdapter);
                    // notify adapter
                    musicAdapter.notifyDataSetChanged();
                } else {
                    Toast.makeText(ListenLater.this, "Error getting documents.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
